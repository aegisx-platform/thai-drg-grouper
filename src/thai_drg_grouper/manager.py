...
class ThaiDRGGrouperManager:
    ...
    def group(
        ...
        if not grouper:
            raise ValueError(f"Version {version} not found. Available: {list(self._versions.keys())}")
    ...
    def group_latest(
        ...
        return self.group(self._default_version, pdx=pdx, sdx=sdx, procedures=procedures, age=age, sex=sex, los=los)
    ...
    def group_all_versions(
        ...
        for version in self._versions:
            try:
                results[version] = self.group(version, pdx=pdx, sdx=sdx, procedures=procedures, age=age, sex=sex, los=los)
            except Exception:
    ...
    def download_version(self, version: str, set_default: bool = False) -> bool:
        ...
        try:
            ...
            success = self.add_version(version=version, source_path=str(temp_zip), set_default=set_default)
            temp_zip.unlink()
            return success
        except Exception:
            if temp_zip.exists():
