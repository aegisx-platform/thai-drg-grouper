...
from .types import GrouperResult, MDC_NAMES
...
class ThaiDRGGrouper:
    ...
    def _find_drg(self, dc: str, pcl: int) -> dict:
        if dc not in self._drg_data:
            return {'drg': '26509', 'name': 'Ungroupable', 'rw': 0, 'rw0d': 0, 'wtlos': 0, 'ot': 0, 'mdc': '26'}
        drgs = sorted(self._drg_data[dc], key=lambda x: x['drg'])
        return drgs[min(pcl, len(drgs) - 1)]
    
    def _calculate_adjrw(self, rw: float, rw0d: float, wtlos: float, ot: int, los: int) -> Tuple[float, str]:
        if los <= 0:
            return rw0d, 'daycase'
        if los <= ot or ot == 0:
            return rw, 'normal'
    ...
    def group(
        ...
        if not pdx_info:
            ...
            return GrouperResult(
                version=self.version, pdx=pdx, sdx=sdx, procedures=procedures,
                age=age, sex=sex, los=los, mdc='26', mdc_name='Ungroupable',
                dc='2650', drg='26509', drg_name='Ungroupable',
                rw=0, rw0d=0, adjrw=0, wtlos=0, ot=0, pcl=0,
                cc_list=[], mcc_list=[], has_or_procedure=False, is_surgical=False,
                los_status='normal', is_valid=False, errors=errors, warnings=warnings,
        ...
        return GrouperResult(
            version=self.version, pdx=pdx, sdx=sdx, procedures=procedures,
            age=age, sex=sex, los=los, mdc=mdc, mdc_name=MDC_NAMES.get(mdc, 'Unknown'),
            dc=dc, drg=drg_info['drg'], drg_name=drg_info['name'],
            rw=drg_info['rw'], rw0d=drg_info['rw0d'], adjrw=adjrw,
            wtlos=drg_info['wtlos'], ot=drg_info['ot'], pcl=pcl,
            cc_list=cc_list, mcc_list=mcc_list, has_or_procedure=has_or,
            is_surgical=is_surgical, los_status=los_status, is_valid=True,
            errors=errors, warnings=warnings, grouped_at=datetime.now().isoformat()
        )
