...
def create_api(manager: ThaiDRGGrouperManager):
    ...
    def group_default(request: GroupRequest):
        ...
        try:
            result = manager.group_latest(
                pdx=request.pdx, sdx=request.sdx, procedures=request.procedures,
                age=request.age, sex=request.sex, los=request.los
            )
    ...
    def group_version(version: str, request: GroupRequest):
        ...
        try:
            result = manager.group(
                version, pdx=request.pdx, sdx=request.sdx, procedures=request.procedures,
                age=request.age, sex=request.sex, los=request.los
            )
            return result.to_dict()
        except ValueError as e:
    ...
    def group_compare(request: GroupRequest):
        ...
        return {
            version: result.to_dict() if result else None
    ...
    def group_batch(
        ...
        version: Optional[str] = Query(None, description="Version to use")
    ):
        """Batch grouping"""
        ...
        for case in request.cases:
            result = manager.group(
                v, pdx=case.pdx, sdx=case.sdx, procedures=case.procedures,
                age=case.age, sex=case.sex, los=case.los
            )
